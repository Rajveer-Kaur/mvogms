...
<div class="container-fluid">
	<div class="row">
        <div class="col-3 border bg-gradient-warning"><span class="text-white font-weight-bold">Reference Code</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($code) ? $code : '' ?></span></div>
        <div class="col-3 border bg-gradient-warning"><span class="text-white font-weight-bold">Vendor</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($vendor) ? $vendor : '' ?></span></div>
        <div class="col-3 border bg-gradient-warning"><span class="text-white font-weight-bold">Client</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($client) ? $ccode.' - '.$client : '' ?></span></div>
        <div class="col-3 border bg-gradient-warning"><span class="text-white font-weight-bold">Delivery Address</span></div>
        <div class="col-9 border"><span class="font-weight-bolder"><?= isset($delivery_address) ? $delivery_address : '' ?></span></div>
        <div class="col-3 border bg-gradient-warning"><span class="text-white font-weight-bold">Status</span></div>
        <div class="col-9 border"><span class="font-weight-bolder">
            <?php 
            $status = isset($status) ? $status : '';
                switch($status){
                    case 0:
                        echo '<span class="badge badge-secondary bg-gradient-secondary px-3 rounded-pill text-white">Pending</span>';
                        break;
                    case 1:
                        echo '<span class="badge badge-warning bg-gradient-warning px-3 rounded-pill text-white">Confirmed</span>';
                        break;
                    case 2:
                        echo '<span class="badge badge-info bg-gradient-info px-3 rounded-pill text-white">Packed</span>';
                        break;
                    case 3:
                        echo '<span class="badge badge-warning bg-gradient-warning px-3 rounded-pill text-white">Out for Delivery</span>';
                        break;
                    case 4:
                        echo '<span class="badge badge-success bg-gradient-success px-3 rounded-pill text-white">Delivered</span>';
                        break;
                    case 5:
                        echo '<span class="badge badge-danger bg-gradient-danger px-3 rounded-pill text-white">Cancelled</span>';
                        break;
                    default:
                        echo '<span class="badge badge-light bg-gradient-light border px-3 rounded-pill text-white">N/A</span>';
                        break;
                }
            ?>
            <?php if($status != 5): ?>
                <span class="pl-2"><a href="javascript:void(0)" id="update_status" class="text-white">Update Status</a></span>
            <?php endif; ?>
        </div>
    </div>
            ...
            <div class="d-flex align-items-center p-2">
                ...
                <div class="col-auto flex-shrink-1 flex-grow-1">
                    ...
                    <div class="d-flex">
                        ...
                        <div class="col-auto px-0 flex-shrink-1 flex-grow-1"><p class="m-0 pl-3"><small class="text-warning font-weight-bold"><?= format_num($prow['price']) ?></small></p></div>
                    </div>
                    <div class="d-flex">
                        <div class="col-auto px-0"><small class="text-muted">Qty: </small></div>
                        <div class="col-auto px-0 flex-shrink-1 flex-grow-1"><p class="m-0 pl-3"><small class="text-warning font-weight-bold"><?= format_num($prow['quantity']) ?></small></p></div>
                    </div>
                </div>
            </div>
        </div>
