...
<!-- Navbar -->
      ...
      <nav class="w-100 px-2 py-1 position-fixed top-0 bg-dark text-light" id="login-nav">
        <div class="d-flex justify-content-between w-100">
          <div>
            <p class="m-0 truncate-1 "><small class="font-weight-bold"><?= $_settings->info('name') ?></small></p>
          </div>
          <div>
            <?php if($_settings->userdata('id') > 0 && $_settings->userdata('login_type') ==3): ?>
              ...
              <div class="dropdown">
                ...
                <span class="mx-2"><img src="<?= validate_image($_settings->userdata('avatar')) ?>" class="img-thumbnail rounded-circle" alt="User Avatar" id="client-img-avatar">  <span class="mx-2 text-warning">Howdy, <?= !empty($_settings->userdata('username')) ? $_settings->userdata('username') : $_settings->userdata('email') ?></span>
                </a>
            ...
            <?php else: ?>
              <a href="./login.php" class="mx-2 text-warning text-decoration-none font-weight-bold">Client Login</a> | 
              <a href="./vendor" class="mx-2 text-warning text-decoration-none font-weight-bold">Vendor Login</a> | 
              <a href="./admin" class="mx-2 text-warning text-decoration-none font-weight-bold">Admin Login</a>
            <?php endif; ?>
          </div>
        </div>
      </nav>
      <nav class="main-header navbar navbar-expand-md navbar-light border-0 text-sm bg-gradient-light shadow" id='top-Nav'>
        ...
        <div class="container">
          <a href="./" class="navbar-brand">
            <img src="<?php echo validate_image($_settings->info('logo'))?>" alt="Site Logo" class="brand-image img-circle elevation-3 mr-3" style="opacity: 1">
